    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl sm:max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Busca y selecciona fuentes existentes para agregar a este cuaderno
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 flex-1 overflow-hidden flex flex-col">
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground truncate">
                          Agregado {formatDate(source.created)}
                        </p>
                      </div>
                    </div>
                  )
          ...
          {allSources.length >= 100 && !debouncedSearchQuery && (
            <div className="text-xs text-muted-foreground bg-muted/50 p-2 rounded-md">
              Mostrando las primeras 100 fuentes. Usa la función de búsqueda para encontrar fuentes específicas.
            </div>
          )}
          ...
          {selectedSourceIds.length > 0 && (
            <div className="text-sm text-muted-foreground">
              {selectedSourceIds.length} fuente{selectedSourceIds.length > 1 ? 's' : ''} seleccionada
            </div>
          )}
        </div>
